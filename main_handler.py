...
class MainHandler(webapp2.RequestHandler):
  ...
  def post(self):
    ...
    operations = {
        ...
        'deleteTimelineItems':self._delete_timeline_items
    }
  ...
  def _delete_timeline_items(self):
    """Delete Timeline Items."""
    logging.info('Deleting timeline items')
    # self.mirror_service is initialized in util.auth.required.
    for item in self.request.get('itemIds').get('items', []):
        logging.info('hello')
    ids = self.request.get('itemIds')
    self.mirror_service.timeline().delete(id=self.request.get('itemIds')).execute()
    return 'Timeline items deleted.'
